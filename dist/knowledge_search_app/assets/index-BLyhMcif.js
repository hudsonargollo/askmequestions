import { C as CredentialsProviderError, s as setCredentialFeature, aT as chain, aO as getProfileName } from "../index.js";
import { p as parseKnownFiles } from "./index-CxP93H2O.js";
import "node:events";
import "node:stream";
import "buffer";
import "path";
...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      const { fromHttp } = await import("./index-3djKRvo0.js");
      const { fromContainerMetadata } = await import("./index-DvLm_qQK.js");
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("./index-DvLm_qQK.js");
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await import("./index-CxP93H2O.js").then((n) => n.i);
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, visitedProfiles = {}) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-CO8sP8HR.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-UbieY64z.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
})().then((creds) => setCredentialFeature(creds, "CREDENTIALS_PROFILE_PROCESS", "v")));
const resolveSsoCredentials = async (profile, profileData, options = {}) => {
  const { fromSSO } = await import("./index-BmKH90jv.js");
  return fromSSO({
    profile,
...
const resolveWebIdentityCredentials = async (profile, options) => import("./index-CAnQWDjg.js").then(({ fromTokenFile }) => fromTokenFile({
