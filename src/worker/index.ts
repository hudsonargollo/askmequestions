...
async function checkRateLimit(c: any, identifier: string): Promise<boolean> {
  try {
...
// Enhanced search endpoint with rate limiting and error handling
app.post('/api/search', zValidator('json', SearchRequestSchema), async (c) => {
  ...
  // Check rate limit
  const rateLimitOk = await checkRateLimit(c, clientIP);
  if (!rateLimitOk) {
    return c.json({
...
app.post('/api/v1/images/generate', authMiddleware, zValidator('json', ImageGenerationRequestSchema), async (c) => {
  ...
  try {
    // Rate limiting check
    const rateLimitOk = await checkRateLimit(c, user.id);
    if (!rateLimitOk) {
      return c.json({
        success: false,
